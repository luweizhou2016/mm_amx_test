...
class Linear16x48_AMX : public jit_generator {
public:
    int m_K;
    TileConfig m_tile_cfg;
    Linear16x48_AMX(int K) : m_K(K) {
        create_kernel("Linear16x48_AMX");
        m_tile_cfg.reset(1, 0,
                         {
                             {16, 64}, // C:0
                             {16, 64}, // C:1
    ...
    Xbyak::Tmm tmmC2 = tmm2;
    Xbyak::Tmm tmmA = tmm3;
    Xbyak::Tmm tmmB0 = tmm4;
    Xbyak::Tmm tmmB1 = tmm5;
    Xbyak::Tmm tmmB2 = tmm6;

    void generate() {
        /*
                       B: 1x3 tiles
        A : 1x1 tiles  C: 1x3 tiles
        */
        Xbyak::Label loop_k_tile;
        assert(m_K % 32 == 0);
        tilezero(tmmC0);
        ...
        if (do_sw_prefetch) {
            for (auto cl = 0; cl < 16; cl++) {
                auto cl_offset = 64 * cl;
                prefetcht0(ptr[reg_B_addr + 2048 + step_b + cl_offset]);

            }
        }
        tileloadd(tmmB2, ptr[reg_B_addr + reg_B_stride + 2048]);
        tdpbf16ps(tmmC2, tmmA, tmmB2);
        ...
        tilestored(ptr[reg_C_addr + reg_C_stride], tmmC0);
        tilestored(ptr[reg_C_addr + reg_C_stride + 64], tmmC1);
        tilestored(ptr[reg_C_addr + reg_C_stride + 2 * 64], tmmC2);
        ret();
    }
};


class Linear48x16_AMX : public jit_generator {
public:
    int m_K;
    TileConfig m_tile_cfg;
    Linear48x16_AMX(int K) : m_K(K) {
        create_kernel("Linear48x16_AMX");
        m_tile_cfg.reset(1, 0,
                         {
                             {16, 64}, // C:0
                             {16, 64}, // C:1
    ...
    Xbyak::Reg64 reg_B_stride = r10;
    Xbyak::Reg64 reg_ktiles = r9;
    Xbyak::Reg64 reg_AX_addr = r11;


    Xbyak::Tmm tmmC0 = tmm0;
    Xbyak::Tmm tmmC1 = tmm1;
    Xbyak::Tmm tmmC2 = tmm2;
    Xbyak::Tmm tmmA0 = tmm3;
    Xbyak::Tmm tmmA1 = tmm4;
    Xbyak::Tmm tmmA2 = tmm5;
    Xbyak::Tmm tmmB = tmm6;

    void generate() {
        /*
                       B: 1x1 tiles
        A : 3x1 tiles  C: 3x1 tiles
        */
        Xbyak::Label loop_k_tile;
        assert(m_K % 32 == 0);
        tilezero(tmmC0);
        ...
        // }
        tileloadd(tmmA0, ptr[reg_A_addr + reg_A_stride]);
        tileloadd(tmmB, ptr[reg_B_addr + reg_B_stride]);
        tdpbf16ps(tmmC0, tmmA0, tmmB);
        ...
        //     }
        // }
        lea(reg_AX_addr, ptr[reg_A_addr + reg_A_stride * 8]);
        lea(reg_AX_addr, ptr[reg_AX_addr + reg_A_stride * 8]);
        tileloadd(tmmA1, ptr[reg_AX_addr + reg_A_stride]);
        tdpbf16ps(tmmC1, tmmA1, tmmB);
        ...
        lea(reg_AX_addr, ptr[reg_AX_addr + reg_A_stride * 8]);
        lea(reg_AX_addr, ptr[reg_AX_addr + reg_A_stride * 8]);
        tileloadd(tmmA2, ptr[reg_AX_addr + reg_A_stride]);
        tdpbf16ps(tmmC2, tmmA2, tmmB);
        ...
        tilestored(ptr[reg_C_addr + reg_C_stride], tmmC1);

        lea(reg_C_addr, ptr[reg_C_addr + reg_C_stride * 8]);
        lea(reg_C_addr, ptr[reg_C_addr + reg_C_stride * 8]);
        tilestored(ptr[reg_C_addr + reg_C_stride], tmmC2);
        ret();
    }
};
